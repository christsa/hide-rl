...
def design_agent_and_env(FLAGS):
    ...
    env = EnvironmentAdapter("fetch", "ball", "simplerandom", project_state_to_end_goal, project_state_to_subgoal, FLAGS.show, normalization_dict, featurize_image=FLAGS.featurize_image, seed=FLAGS.seed)
    eval_env = EnvironmentAdapter("fetch", "ball", "simple", project_state_to_end_goal, project_state_to_subgoal, FLAGS.show, normalization_dict, featurize_image=FLAGS.featurize_image, seed=FLAGS.seed)
    ...
    agent_params["subgoal_noise"] = [0.02 for i in range(len(env.subgoal_thresholds))]
    agent_params["oracle_noise"] = [0.02 for i in range(len(env.subgoal_thresholds))]
