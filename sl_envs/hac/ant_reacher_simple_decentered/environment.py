...
class Environment():
    ...
    def __init__(self, model_name, goal_space_train, goal_space_test, project_state_to_end_goal, end_goal_thresholds, initial_state_space, initial_state_space_test, subgoal_bounds, project_state_to_subgoal, subgoal_thresholds, max_actions = 1200, num_frames_skip = 10, show = False):

        self.name = model_name

        # Create Mujoco Simulation
        self.model = load_model_from_path(os.path.join(MODULE_DIR, model_name))
        ...
        self.initial_state_space_test = initial_state_space_test
    ...
    def reset_sim(self, next_goal = None, test=False):

        # Reset controls
        self.sim.data.ctrl[:] = 0
        initial_state_space = self.initial_state_space_test if test else self.initial_state_space

        if self.name == "ant_reacher.xml":
            while True:
                # Reset joint positions and velocities
                for i in range(len(self.sim.data.qpos)):
                    self.sim.data.qpos[i] = np.random.uniform(initial_state_space[i][0],initial_state_space[i][1])

                for i in range(len(self.sim.data.qvel)):
                    self.sim.data.qvel[i] = np.random.uniform(initial_state_space[len(self.sim.data.qpos) + i][0],self.initial_state_space[len(self.sim.data.qpos) + i][1])
    ...
    def execute_action(self, action):
        ...
        for _ in range(self.num_frames_skip):
            self.sim.step()
            # if self.visualize:
            #     for subgoal_ind in range(len(self._subgoals)):
            #         self.viewer.add_marker(pos=[self._subgoals[subgoal_ind][0], self._subgoals[subgoal_ind][1], 1], label=f'level:{subgoal_ind}, {self._subgoals[subgoal_ind][:2]}')
            #     if self._end_goal is not None:
            #         self.viewer.add_marker(pos=[self._end_goal[0], self._end_goal[1], 1], label=(str(self._end_goal)))
            #     ant_pos = self.sim.data.qpos[:2]
            #     self.viewer.add_marker(pos=[ant_pos[0], ant_pos[1], 1], label=("ant"+str(ant_pos)+", reward:"+str(
            #         -np.sqrt(np.sum(np.square(self._end_goal - ant_pos))+1e-8)
            #         )))
            #     self.viewer.render()
