...
def design_agent_and_env(FLAGS):
    ...
    normalization_dict = None if not FLAGS.normalization else {
        'lows': OrderedDict((
            (0, -14.),  # xpos
            (1, -14.),  # ypos
            (2, None),  # zpos do not normalize
            (15, None),  # xvel do not normalize
            (16, None),  # yvel do not normalize
        )),
        'highs': OrderedDict((
            (0, 14.),  # xpos
            (1, 14.),  # ypos
        ...
        'end_goal_dims': [0,1],
        'subgoal_dims': [0,1,2,15,16],
    }

    project_state_to_end_goal = lambda sim, state: state[:2]
    project_state_to_subgoal = lambda sim, state: np.concatenate((state[:2], np.array([1 if state[2] > 1 else state[2]]), np.array([3 if state[i] > 3 else -3 if state[i] < -3 else state[i] for i in range(15, 17)])))
    env = EnvironmentAdapter("maze", "Ant", "KeyGateHACDict", project_state_to_end_goal, project_state_to_subgoal, FLAGS.show, normalization_dict, featurize_image=FLAGS.featurize_image, seed=FLAGS.seed)
    ...
    agent_params["oracle_noise"] = [0.01 for i in range(env.end_goal_dim)]
