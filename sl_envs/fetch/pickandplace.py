...
class PushEnv(fetch_env.FetchEnv, gym.utils.EzPickle):
    def __init__(self, maze_structure):
        initial_qpos = {
            ...
            'robot0:shoulder_lift_joint': - 0.55,
            'robot0:upperarm_roll_joint': 0.0,
            'robot0:elbow_flex_joint': 1.2,
        ...
        model_file_handle, model_file_path = self.create_maze(self.center, maze_structure,
        ...
        self.observation_space = gym.spaces.Dict({
            'observation': gym.spaces.Box(-np.inf, np.inf, shape=(len(self._get_obs()['observation']),),
                                          dtype=np.float32),
    ...
    def is_pos_valid(self, pos):
        if (pos < -0.2).any() or (pos > 0.2).any():
            print(f"Returning invalid position: {pos}")
            return False
        else:
            return True
    ...
    def _reset_sim(self):
        ...
        if self.object_coords is None:
            ...
            x, y = None, None
            while x is None or self.MAZE_STRUCTURE[len(self.MAZE_STRUCTURE) - x - 1][y] != 0 or (
                    (x - center_x) == int(len(self.MAZE_STRUCTURE) / 2) and (y - center_y) == int(
                    len(self.MAZE_STRUCTURE[0]) / 2)):
                x, y = np.random.randint(2, len(self.MAZE_STRUCTURE) - 2), np.random.randint(2, len(
                    self.MAZE_STRUCTURE[0]) - 2)
                self.object_init_pos = (x - center_x, y - center_y)
                object_xpos = ((y - center_y) * self.MAZE_SIZE_SCALING + self.center[0],
                               (x - center_x) * self.MAZE_SIZE_SCALING + self.center[1])
        else:
            object_xpos = (self.object_coords[1] * self.MAZE_SIZE_SCALING + self.center[0],
                           self.object_coords[0] * self.MAZE_SIZE_SCALING + self.center[1])
    ...
    def _sample_goal(self):
        ...
        if self.target_coords is None:
            ...
            x, y = None, None
            while x is None or self.MAZE_STRUCTURE[len(self.MAZE_STRUCTURE) - x - 1][y] != 0 or (
                    (x - center_x) == self.object_init_pos[0] and (y - center_y) == self.object_init_pos[1]):
                # Keep margins of size 2 when sample.
                x, y = np.random.randint(2, len(self.MAZE_STRUCTURE) - 2), np.random.randint(2, len(
                    self.MAZE_STRUCTURE[0]) - 2)
                self.goal[:2] = ((y - center_y) * self.MAZE_SIZE_SCALING + self.center[0],
                                 (x - center_x) * self.MAZE_SIZE_SCALING + self.center[1])
                # self.target_coords = (x-center_x, y-center_y)
        else:
            self.goal[:2] = (self.target_coords[1] * self.MAZE_SIZE_SCALING + self.center[0],
                             self.target_coords[0] * self.MAZE_SIZE_SCALING + self.center[1])
    ...
    def _render_subgoals(self, viewer, subgoals):
        ...
        for level, subgoal in subgoals.items():
            ...
            if subgoal.size == 2:
                ...
                viewer.add_marker(pos=[subgoal[0] + self.center[0], subgoal[1] + self.center[1], 0.45], label='',
                                  size=[0.01, 0.01, 0.01], rgba=colors[level])
            else:
