...
def design_agent_and_env(FLAGS):
    ...
    FLAGS.variable_env = True
    project_state_to_end_goal = lambda sim, state: state[..., :2]
    project_state_to_subgoal = lambda sim, state: state[..., :2]
    env = EnvironmentAdapter("fetch", "push", "random_no_fence", project_state_to_end_goal, project_state_to_subgoal, FLAGS.show, normalization_dict, featurize_image=FLAGS.featurize_image, seed=FLAGS.seed)
    # eval_env = EnvironmentAdapter("fetch", "push", "simple", project_state_to_end_goal, project_state_to_subgoal, FLAGS.show, normalization_dict, featurize_image=FLAGS.featurize_image)
    ...
    agent_params["atomic_noise"] = [0.2 for i in range(env.action_dim)]
    agent_params["subgoal_noise"] = [0.05 for i in range(len(env.subgoal_thresholds))]
    agent_params["oracle_noise"] = [0.05 for i in range(len(env.subgoal_thresholds))]
    agent_params["vpn_noise"] = [0.1 for i in range(len(env.subgoal_thresholds))]
